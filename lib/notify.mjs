...
export function createNotifier({
  ...
  // The bot token travels in the request URL PATH, so whoever sets apiBase decides where
  // the credential is sent. This override exists for one reason -- pointing an end-to-end
  // test at a local stub -- and a stale value left in a shell or copied into a .env would
  // otherwise ship the token to an arbitrary host while alerting still looked healthy (a
  // stub returning {"ok":true} is indistinguishable from Telegram accepting it). Never
  // silent, therefore, even though the value itself is not a credential.
  if (enabled && apiBase !== TELEGRAM_API_BASE) {
    safeLog(
      `notify: WARNING — sending to a NON-DEFAULT endpoint (${apiBase}) instead of ` +
        `${TELEGRAM_API_BASE}. The bot token is part of that URL. Unset GPA_TELEGRAM_API_BASE ` +
        'unless you are deliberately testing against a stub.',
    );
  }
