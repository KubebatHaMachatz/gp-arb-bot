...
/**
 * Parse the minimum Node version out of a `package.json` `engines.node` range.
 *
 * The installer needs this to reject a `node` that cannot run the services, and it must
 * come from `engines` rather than a second literal. The two silently disagreeing is a
 * live failure mode, not a tidiness point: when `--env-file-if-exists` raised the floor
 * to 22.9, a hardcoded 22.5 would have let the installer select a 22.5-22.8 binary, write
 * plists passing a flag that Node rejects, and leave three services crash-looping with
 * the cause visible only in the system log.
 *
 * Only the `>=X.Y` form this repo uses is accepted. A shape it cannot read throws rather
 * than guessing a floor and getting it wrong in the permissive direction.
 *
 * @param {string} range e.g. `'>=22.9'`
 * @returns {[number, number]} `[major, minor]`
 * @throws {TypeError} on a range this cannot parse
 */
export function parseEnginesFloor(range) {
  const m = /^>=\s*(\d+)(?:\.(\d+))?/.exec(String(range ?? '').trim());
  if (!m) {
    throw new TypeError(
      `cannot read a minimum Node version from engines.node ${JSON.stringify(range)}; ` +
        'expected the form ">=22.9"',
    );
  }
