...
export async function discoverMarkets({
  ...
  for (let page = 0; page < maxPages; page += 1) {
    ...
    for (const event of body.events) {
      for (const row of normalizeEvent(event)) rows.push({ ...row, fetchedAtMs });
    ...
    // An empty or repeated cursor means the end; without the repeat check a server that
    // echoes the same cursor would page forever.
