 ...
 *   is not.
 * - **A `clear` claim is restricted to `kind = 'binary'`.** A neg-risk group's
 *   completeness rests on Polymarket's `negRiskMarketID` plus a matching leg count —
 *   which is not proof the outcomes are exhaustive. A live 9-leg "Eurozone inflation"
 *   event was found with bands that do not tile the real number line (1.9 sits in two
 *   bands; 1.6 and 2.2–2.3 sit in none), so a "complete" set built from it can redeem
 *   for $0, not $1. A neg-risk row is still PRICED here — it carries a real, storable
 *   `net_edge` and is never hidden — it just cannot claim to be a genuine, redeemable
 *   `clear` until a real exhaustiveness check exists. `bestNetEdge` and
 *   `clearingCapacityUsd` are scoped the same way, for the same reason: showing an
 *   untrusted neg-risk figure as "how close the market came" would be a worse kind of
 *   misleading than not gating it at all.
 */

import { KIND_BINARY } from './arb.mjs';

/** The `net_edge >= ? AND kind = ?` predicate every clear-claiming query shares. */
const CLEAR_PREDICATE = 'net_edge >= ? AND kind = ?';
...
/**
 * True median of an ASCENDING array, averaging the middle pair on even lengths.
 *
 * @param {ReadonlyArray<number>} sorted
 * @returns {number|null} `null` for an empty set — a median of nothing is not 0
 */
  ...
  const row = db
    .prepare(
      `SELECT
         ...
         SUM(CASE WHEN ${CLEAR_PREDICATE} THEN 1 ELSE 0 END)                AS clears,
         MAX(CASE WHEN kind = ? THEN net_edge ELSE NULL END)                AS best_net_edge,
         SUM(CASE WHEN ${CLEAR_PREDICATE} THEN capacity_usd ELSE 0 END)     AS clearing_capacity
       FROM opportunities ${clause}`,
    )
    .get(floor, KIND_BINARY, KIND_BINARY, floor, KIND_BINARY, ...params);
  ...
  return db
    .prepare(
      `SELECT
         ...
         COUNT(DISTINCT CASE WHEN o.net_edge >= ? AND o.kind = ? THEN o.id END) AS clears,
         MAX(CASE WHEN o.kind = ? THEN o.net_edge ELSE NULL END)            AS best_net_edge,
         SUM(CASE WHEN o.net_edge >= ? AND o.kind = ? THEN o.capacity_usd ELSE 0 END) AS clearing_capacity
    ...
    .all(floor, KIND_BINARY, KIND_BINARY, floor, KIND_BINARY, ...params)
    .map((r) => ({
      category: r.category,
      priced: r.priced,
      clears: r.clears,
...
export function capacityBinding(db, { sinceMs, venue } = {}) {
  ...
  return {
    ...
    medianCapacityUsd: median(sizes),
  };
}
...
export function recentClears(db, { sinceMs, venue, minNetEdge, limit = 50 } = {}) {
  ...
  const { clause, params } = windowClause({ sinceMs, venue });
  const where = clause ? `${clause} AND ${CLEAR_PREDICATE}` : `WHERE ${CLEAR_PREDICATE}`;
  ...
  return db
    ...
    .all(...params, floor, KIND_BINARY, bounded)
    .map((r) => ({
      id: r.id,
      venue: r.venue,
