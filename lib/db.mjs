...
export function sweepOpportunities(db, { nowMs, keepDays }) {
  ...
  const before = db.prepare('SELECT COUNT(*) AS n FROM opportunities WHERE ts < ?').get(cutoffMs);
  db.prepare('DELETE FROM opportunities WHERE ts < ?').run(cutoffMs);
  return { deleted: Number(before.n), cutoffMs };
}
