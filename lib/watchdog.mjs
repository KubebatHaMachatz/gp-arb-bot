 ...
 *
 * `readFailed` is reported explicitly and NOT as "watching nothing". A watchdog that
 * comes up unable to read the database is the case an operator most needs to hear about,
 * and it must not be dressed up as a quiet-but-healthy start.
 *
 * @param {{venues: ReadonlyArray<string>, feeds: ReadonlyArray<object>,
 *          keepOppDays: number, nowMs: number, readFailed?: boolean}} args
 * @returns {string}
 */
export function startupMessage({ venues, feeds, keepOppDays, nowMs, readFailed = false }) {
  ...
  let watched;
  if (readFailed) {
    watched = '  COULD NOT READ THE DATABASE — feed state is unknown. The watchdog is up but blind.';
  } else if (venues.length === 0) {
    watched = '  (no venues being watched — nothing has written rows)';
  } else {
    watched = lines.join('\n');
  }
