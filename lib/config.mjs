...
export const DEFAULTS = Object.freeze({
  ...
  GPA_CLOCK_SKEW_TOLERANCE_MS: 5000,
  ...
  GPA_STARTUP_PING_MIN_GAP_MS: 60000,
...
/**
 * Upper bound on the clock-skew tolerance — the other side of the freshness gate.
 *
 * `bookAgeMs > cfg.bookStaleMs` can never be tripped by a negative age, however large
 * in magnitude, so a book whose timestamp looks anomalously far in the future (an NTP
 * glitch, a corrupted field, a units mismatch) is otherwise silently treated as
 * "maximally fresh" with no error. `clockSkewToleranceMs` closes that hole, but only if
 * it too is bounded above: an unbounded tolerance is the same silent disarming
 * `MAX_BOOK_STALE_MS` exists to prevent, just on the other side of zero. Live
 * Polymarket data measured the venue's timestamp running up to ~190ms ahead of this
 * process's clock — ordinary jitter — so one minute is already a wide margin above
 * anything defensible as jitter.
 */
const MAX_CLOCK_SKEW_TOLERANCE_MS = 60_000;
...
export function loadConfig(env = process.env) {
  ...
  return Object.freeze({
    ...
    /**
     * The other side of the freshness gate: how far the venue's own timestamp may run
     * AHEAD of this process's clock before it is treated as anomalous rather than
     * ordinary jitter. `0` is valid — zero tolerance for any future-looking timestamp is
     * a legitimate, if strict, choice.
     */
    clockSkewToleranceMs: parseIntegerEnv(env.GPA_CLOCK_SKEW_TOLERANCE_MS, {
      name: 'GPA_CLOCK_SKEW_TOLERANCE_MS',
      def: DEFAULTS.GPA_CLOCK_SKEW_TOLERANCE_MS,
      min: 0,
      max: MAX_CLOCK_SKEW_TOLERANCE_MS,
    }),
    ...
    /**
     * Minimum gap between startup announcements.
     *
     * Separates a deliberate restart (announce) from a launchd KeepAlive crash loop
     * (do not announce every ten seconds until the channel is muted). `0` announces
     * every start.
     */
    startupPingMinGapMs: parseIntegerEnv(env.GPA_STARTUP_PING_MIN_GAP_MS, {
      name: 'GPA_STARTUP_PING_MIN_GAP_MS',
      def: DEFAULTS.GPA_STARTUP_PING_MIN_GAP_MS,
      min: 0,
    }),
