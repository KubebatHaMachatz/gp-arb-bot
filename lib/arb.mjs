 ...
 *
 * Only `KIND_BINARY` can clear. A neg-risk group's completeness rests on Polymarket's
 * own `negRiskMarketID` plus a matching leg count, which is not proof the outcomes are
 * exhaustive — a live 9-leg "Eurozone inflation" event was found with bands that do not
 * tile the real number line (1.9 sits in two bands; 1.6 and 2.2–2.3 sit in none), so a
 * "complete" set built from it can redeem for $0, not $1. The edge is still returned by
 * `detectOpportunity` as a real, storable `netEdge` — this is not a skip, and nothing
 * here hides the data — it just cannot claim to be a genuine, redeemable clear until a
 * real exhaustiveness check exists. Written as a positive whitelist rather than a
 * blacklist on `KIND_NEG_RISK` specifically, so a third kind added later — or a caller
 * that simply forgot to set `kind` — fails closed by default rather than being silently
 * treated as clearable, the same shape this file's freshness gate was already fixed
 * twice for.
 *
 * @param {{netEdge: number|null, skipped?: string|null, kind?: string}} result
...
export function clearsThreshold(result, { minNetEdge }) {
  ...
  if (result.kind !== KIND_BINARY) return false;
