...
export function detectOpportunity({ venue, eventKey, kind, legs, feeFn, cfg, nowMs, bookTs }) {
  ...
  // Validated explicitly because this one fails OPEN. Every other cfg field read here
  // throws when absent (minNetEdge via clearsThreshold, depthSafetyFactor and
  // maxSetSizeUsd via setCapacity), but `age > undefined` is a NaN comparison and
  // therefore false — a partial or misspelled cfg would silently disable the freshness
  // gate and publish tradeable claims on arbitrarily stale books, with no error
  // anywhere. The gate that guards against acting on a quote that is already gone must
  // fail closed.
  if (
